...
class AddCollectionProjectPop extends React.Component {
  ...
  async startSearch() {
    ...
    if(searchByUrl){  
      ...
      nonCollectionResults = results.filter(result => result ? result.domain == query : null);
  ...
  render() {
    ...
    const results = this.state.query ? this.state.maybeResults : (ownProjects ? ownProjects.slice(0,20) : null); // TO DO - comment out
