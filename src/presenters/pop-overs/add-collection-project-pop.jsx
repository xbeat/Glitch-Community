...
export class AddCollectionProjectPop extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      templateProjects: [],
  ...
  updateFilter(query) {
    ...
    let filteredProjects = this.filterProjects(query, projects, this.props.collectionProjects);
    this.setState({
      filteredProjects: filteredProjects
    });
  }

  filterProjects(query, projects, collectionProjects) {
    query = query.toLowerCase().trim();
    let MAX_PROJECTS = 20;
    let teamProjectIds = collectionProjects.map(({id}) => id);
...
const AddCollectionProjectPopContainer = (props) => (
  <CurrentUserConsumer>
    {currentUser => <AddCollectionProjectPop myProjects={currentUser.projects} {...props}/>}
  </CurrentUserConsumer>
);

export default AddCollectionProjectPopContainer;
