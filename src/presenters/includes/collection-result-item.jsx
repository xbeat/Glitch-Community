...
const addProject = (api, project, collection, collectionPath, notification, togglePopover) => {
    ...
    <Notifications>
      {({createNotification}) => (
        <DataLoader get={() => getCollectionUrl(api, collection.userId, collection.url)}>
          {collectionPath => 
            <div>
              <button className={resultClass} onClick={() => addProject(api, project, collection, collectionPath, createNotification, togglePopover)} data-project-id={project.id}>
